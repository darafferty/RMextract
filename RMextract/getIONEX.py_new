...
def _read_ionex_header(filep):
    ...
    Returns:
        tuple: multiplication factor,lonarray,latarray,timearray
    """
    ...
    if timearray.shape[0] < ntimes:
        ...
        extratimes = np.arange(timearray[-1] + timestep, 
                                timearray[-1] 
                                + (ntimes-timearray.shape[0] + 0.5)*timestep,
                                timestep)
...
def read_tec(filename,_use_filter=None):
    ...
    Returns:
        tuple: 3D-arrays (time,lat,lon) of (optionally filtered) TEC and RMS +
    ...
    logging.info("reading data with shapes %d  x %d x %d"%(timearray.shape[0],
                                                           latarray.shape[0],
                                                           lonarray.shape[0]))
    ...
    if not _use_filter is None:
        tecdata = myfilter.gaussian_filter(
                  tecdata,_use_filter,mode='nearest')
...
def _compute_index_and_weights(maparray,mapvalues):
    ...
    Args:
        maparray(array of float) : array to get indices in
        mapvalues(float or array of float) :  values to get indices for
    Returns:
        idx1,idx2 and weights for idx2, idx2 is always >= idx1
...
def compute_tec_interpol(times,lats,lons,tecinfo,apply_earth_rotation=0):
    ...
    Args:
        lats (array of float) : angles in degrees between -90 and 90
        lons (array of float) : angles in degrees between -180,180
        times (array of float) : times in decimal hour of the day (eg. 23.5 
        for half past 11PM)
    ...
    maptimes = tecinfo[4]; # times in hour of the day (eg. 23.5 for half past 11PM)
    ...
    logging.debug("inidces time %d %d indices lat %d %d indices \
                  ...
                  latidx1[0],latidx2[0],lonidx11[0],lonidx12[0],
                  lonidx21[0],lonidx22[0]) )
    logging.debug("weights time %f lat %f lon %f %f"%(time_weights[0],
                                                      lat_weights[0],
                                                      lon_weights1[0],
                                                      lon_weights2[0]))
...
def _store_files(ftp,filenames,outpath,overwrite=False):
    ...
    for myf in filenames:
        mypath = os.path.join(outpath,myf)
        if not overwrite and os.path.isfile(mypath):
            nfilenames.append(mypath)
            logging.info("file %s exists"%mypath)
        elif not overwrite and os.path.isfile(mypath.strip(".Z").strip(".z")):
            nfilenames.append(mypath.strip(".Z").strip(".z"))
            logging.info("file %s exists"%mypath.strip(".Z").strip(".z"))
        else:
            myp = open(mypath,"wb")
    ...
    for myf in nfilenames_copy:
        if myf.endswith(".Z") or myf.endswith(".z"):
            nfilenames.append(_gunzip_some_file(myf,
                                                myf.strip(".Z").strip(".z")))
        else:
...
def _get_IONEX_file(time="2012/03/23/02:20:10.01",
                    ...
                    outpath='./',
                    overwrite=False):
    """Get IONEX files for given prefix from server"""
    ...
    if not os.path.isdir(outpath):
        try:
            ...
            logging.error("cannot create output directory for IONEXdata",
                          outpath)
            raise
    try:
        yy=time[2:4];
